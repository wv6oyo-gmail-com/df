@	My modified jonesforth.
@
@	dF --  draper Forth

	.include "vector-table.s"
...
PSP	.req  r6
N       .req  r7
I	.req  r4    @ push FIP onto FORTH stack then use I
V	.req  r5    @ push RSP onto FORTH stack then use V
...
*/
	.macro NEXT
	ldr W,[FIP,#0]        @ load W with next address
	add FIP,FIP,#4        @ bump INSTRUCTION POINTER
	ldr X,[W,#0]	      @ fetch address to execute
	add X,X,#1	      @ thumb instruction thing !!! 
	BX X                  @ jump to cyber space
	.endm

next:	ldr W,[FIP,#0]        @ load W with next address
	add FIP,FIP,#4        @ bump INSTRUCTION POINTER
	ldr X,[W,#0]	      @ fetch address to execute
	add X,X,#1	      @ thumb instruction thing !!! 
	BX X                  @ jump to cyber space
...
+++  Macros to deal with the return stack.
	...
	.endm
*/
@ ParameterStack grows up
@ PSP points to top of PS
@ Note that the stack pointer is incremented BEFORE data is stored
...
@ ReturnStack grows down
@ RSP points to top of return stack
...
*/
	...
	.set RAM_BOTTOM, 0x20000000
	.set RAM_TOP,    0x20010000
	.set TIB_ADDR,   RAM_TOP-512-128
	.set PSP0,       RAM_TOP-512-4      @ 512/4 = 128 cells
	.set RSP0,       RAM_TOP
...
/*
         ...
         2000.0000       | TIB      | PARAMETER STACK          | RETURN STACK             |
	+----------------+----------+--------+--------+--------+--------+--------+--------+
	|  . . .         | 128 BYTES|         64 CELLS         |         64 CELLS         |
	+----------------+----------+--------+--------+--------+--------+--------+--------+
                                     |                                                     |
	                             FE00                                                  1.0000
...
name_\label :
	...
	.set link,name_\label
	.byte \flags+\namelen	@ flags + length byte
	.asciz "\name"		@ the name  NOTE: I'm using null terminated string here
	                        @ makes it easier to print
	.align 2,0xFF		@ padding to next 4 byte boundary
	.globl \label
\label :
...
@	NEXT
	...
	defcode "3DROP",5,,THREEDROP @ drop top two elements of stack
	sub PSP,PSP,#4
	sub PSP,PSP,#4
	sub PSP,PSP,#4
	NEXT
...
@	NEXT
	...
	defcode "3DUP",4,,THREEDUP @( w x y -- w x y w x y )
	mov W,PSP
	sub W,W,#8
	ldr X,[W]
	pushPSP X
	...
	defcode "2DUP",4,,TWODUP @ ( x y -- x y x y )
	mov W,PSP
	sub W,W,#4
	ldr X,[W]
	pushPSP X
...
@	NEXT
	...
	defcode "?DUP",4,,QDUP	@ ( x -- x x | 0 -- 0 ) duplicate top of stack if non-zero
	popPSP W
	cmp W,#0
	beq qdup
	pushPSP W
...
@
@	mov	Z, #0		@ q
	mov	r8,Y		@ save a copy of Y in r8
	mov	r9,X 		@ save a copy of X in r9
	movw	Z, #0		@ q = 0
	@ subtract Y from X repeatedly
1:
_divmod1:	
	sub	X, X, Y	        @ x = x - y
	bpl	_divmod2
	b	_divmod3
_divmod2:
	add	Z, Z, #1	@ if x > 0: q = q + 1
	b	_divmod1	@ until x < 0
_divmod3:	
	@ quotient in Z now, compute remainder
	mov	W,Z             @ save quotient in W
	mov	X,r9            @ restore X from r9
	mul	Z,Y,Z		@ X = y * q
	sub	Y,X,Z		@ r = x - y * q
...
2:
	@ division by 0 error
	mov	X, #stderr
	ldr	Y, =divmsg		@ address
	mov	Z, #divmsgend-divmsg	@ length
	svc	__NR_write
	mov	X, #-1
	svc	__NR_exit
...
3:
	@ division by 0 error
	mov	X, #stderr
	ldr	Y, =posmsg		@ address
	mov	Z, #posmsgend-posmsg	@ length
	svc	__NR_write
	mov	X, #-1
	svc	__NR_exit
...
*/
	...
	defword "MOD",3,,MOD @( x y -- z ) z = x % y
	.int DIVMOD,DROP
	.int EXIT

	defword "/",1,,DIV
	.int DIVMOD,SWAP,DROP
	.int EXIT
/*
...
*/
	...
	defcode "=",1,,EQU	@ ( x y -- f )top two words are equal?
	popPSP W
	popPSP X
	cmp X,W
	beq equ
nequ:	loadREG W,0x00000000
	pushPSP W
	NEXT
equ:	loadREG W,0xFFFFFFFF
	pushPSP W
	NEXT
	...
	defword "MIN",3,,MIN @( 1 2 -- 1 )
	.int TWODUP,LT   @( x y f )
	.int ZBRANCH,min9
	.int DROP,EXIT
min9:	.int SWAP,DROP,EXIT
	...
	cmp W,#0
	beq zequ
zequx:	loadREG W,0x00000000
	pushPSP W
	NEXT
zequ:	loadREG W,0xFFFFFFFF
	pushPSP W
	NEXT
...
*/
	...
	defcode "EXIT",4,,EXIT
	popRSP FIP		@ pop return stack into FIP
	NEXT
...
*/
	...
	defcode "!",1,,STORE   @ ( x addr -- )
	popPSP W
	popPSP X
	str X,[W]
	NEXT

	defcode "@",1,,FETCH  @ ( addr -- x )
	ldr W,[PSP]
	ldr W,[W]
	str W,[PSP]
	NEXT

	defcode "+!",2,,ADDSTORE @ ( x addr -- )
	popPSP X   @ X=addr
	popPSP W   @ W=x
	ldr Y,[X]  @ Y=*addr
	add Y,W,Y
 ...
 */
	...
	defcode "C!",2,,STOREBYTE  @ ( c addr -- )
	popPSP W
	popPSP X
	strb X,[W]
	NEXT

	defcode "C@",2,,FETCHBYTE @ ( addr -- c )
	popPSP W
	ldrb W,[W]
	pushPSP W
	NEXT
...
/* and CMOVE is a block copy operation. */
	defcode "CMOVE",5,,CMOVE   @ ( from to n -- )
	popPSP N
	popPSP W    @ get to address
	popPSP X    @ get from address
move1:	cmp N,#0
	beq cmove9
	ldrb Y,[X]
	strb Y,[W]
...
cmove9:	NEXT
	...
	defcode "ALIGN",5,,ALIGN  @ ( x -- y ) y is aligned to 4 byte boundary
	ldr W,[PSP]
	add W,W,#3
	loadREG X,0xFFFFFFFC @ FC => 1111 1100
	and W,W,X
	str W,[PSP]
	NEXT
...
var_\name :
	...
	.endm
*/
	.set DICTIONARY_SPACE,0x20000200
	.set dataspace,0x20000100
	.set tib,0x20000000
	
	.macro defvar name,namelen,flags=0,label,addr
	defcode \name,\namelen,\flags,\label
@	loadREG W,\addr
	loadREG W,dataspace
	pushPSP W
	.set var_\name,dataspace
	.set dataspace,(dataspace+4)
	NEXT
	.endm
/*
NOTE:	I've changed the defvar macro. The addr parameter is now specified when I declare
	a variable. For now my variable begin at 0x2000.0100. Each of the variable will
	be assigned a value when the system boots (see COLD)
         2000.0000
	+---------+-----------------+--------+--------+--------+--------+--------+--------+
...
*/
	defvar "STATE",5,,STATE,0x20000100
	defvar "HERE",4,,HERE,0x20000104
	defvar "LATEST",6,,LATEST,0x20000108
	defvar "BASE",4,,BASE,0x20000110
	defvar "IN",2,,IN,0x00000000
	defvar "x",1,,X,0  @ scratch variable
	defvar "n",1,,N,0  @ scratch variable
	defvar "SPAN",4,,SPAN,0
	defvar "NTIB",4,,NTIB,0
	defvar "TOIN",4,,TOIN,0
	defvar "HLD",3,,HLD
	defvar "PAD",3,,PAD
	.set dataspace,(dataspace+32)

	@ Before using C>PAD be sure to initialize PAD => PAD DUP 4+ SWAP !
	
	defcode "C>PAD",5,,CTOPAD @( c -- )
	popPSP X
	loadREG W,var_PAD
	ldr Z,[W]
	strb X,[Z]
...
*/
	...
	.macro defsconst name, namelen, flags=0, label, value
	defcode \name,\namelen,\flags,\label
	movw W,\value
	pushPSP W
	NEXT
...
@ Most of these constants are "short" so the macro can be optimized with movw W,\value

	defconst "VERSION",7,,VERSION,JONES_VERSION
	defconst "R0",2,,RZ,RSP0
	defconst "S0",2,,SZ,PSP0
	defconst "DOCOL",5,,__DOCOL,DOCOL
	defconst "F_IMMED",7,,__F_IMMED,F_IMMED
	defconst "F_HIDDEN",8,,__F_HIDDEN,F_HIDDEN
	defconst "F_LENMASK",9,,__F_LENMASK,F_LENMASK
	defconst "TIB",3,,TIB,TIB_ADDR

@ these are "short" constants
	
	defsconst "BELL",4,,BELL,7
	defsconst "BS",2,,BS,8
	defsconst "BL",2,,BL,0x20 @ space
	defsconst "0",1,,ZERO,0
	defsconst "1",1,,ONE,1
	defsconst "2",1,,TWO,2
	defsconst "3",1,,THREE,3
	defsconst "4",1,,FOUR,4
	defsconst "5",1,,FIVE,5
	defsconst "6",1,,SIX,6
	defsconst "7",1,,SEVEN,7
	defsconst "8",1,,EIGHT,8
	defsconst "9",1,,NINE,9
	defsconst "A",1,,TEN,10
	defsconst "B",1,,ELEVEN,11
	defsconst "C",1,,TWELVE,12
	defsconst "D",1,,THIRDTEEN,13
	defsconst "E",1,,FOURTEEN,14
	defsconst "F",1,,FIFTEEN,15
...
*/
	...
	defcode ">R",2,,TOR       @ ( x -- ) R( -- x )
	popPSP W
	pushRSP W
	NEXT

	defcode "R>",2,,FROMR     @ ( -- x ) R( x -- )
	popRSP W
	pushPSP W
	NEXT

	defcode "R@",2,,RFETCH  @( -- x ) R( -- )
	ldr W,[RSP]
	pushPSP W
	NEXT
	
	defcode "RSP@",4,,RSPFETCH @ ( -- x )
	mov W,RSP
	pushPSP W
	NEXT
	...
	defcode "PSP@",4,,PSPFETCH  @ ( -- x )
	mov W,PSP
	pushPSP W
	NEXT

	defcode "PSP!",4,,PSPSTORE @ ( x -- )
	popPSP W
	mov PSP,W
	NEXT

	defcode "?STACK",6,,QSTACK @( -- f )
pspunder: @( -- f )
	movw X,#0
	loadREG W,PSP0
	cmp PSP,W
...
@
/*
	The Luminary UART has a flags register (UARTFR) at address 0x4000C0018.
	UARTFR => XXXX XXXX XXXX XXXX XXXX XXXX 1001 0XXXX
	                                        |||| |
	                                        |||| +-- 1 = BUSY
	                                        |||+---- 1 = RXFE receive FIFO empty
	                                        ||+-----
	                                        |+------
	                                        +------- 1 = TXFE
*/
	defcode "getchar",6,,getchar @( -- c )
	loadREG X,0x4000C000
	ldr W,[X]            @ fetch character
	pushPSP W            @ put it on the stack
	NEXT
	
	defcode "(key?)",6,,keyq  @( -- f ) 
	loadREG X,0x4000C000 @ point X at UART
	ldr W,[X,#0x018]     @ load W with UARTFR
	movw Y,#0x0010       @ load Y with RXFE mask
	and W,W,Y            @ mask off RXFE
	cmp W,#0x0010        @ compare
	beq nokey            @ if W == 0x0010 then
	movw W,#0
	mvn W,W
	pushPSP W
	NEXT
nokey:
	movw W,#0
	pushPSP W            @ put ZERO onto stack	
	NEXT

	
	defword "(KEY)",5,,KEYQ  @( -- c )
KEYQ1:	.int keyq          @( f )
	.int ZBRANCH,KEYQ1
	.int getchar       @ ( c )
	.int EXIT
	...
	defcode "KEY",3,,KEY @ ( -- x ) blocks until character is present !!!
voodoo:	loadREG X,0x4000C000
key1:	ldr W,[X,#0x018]     @ load W with UARTFR
	movw Y,#0x0010       @ load Y with RXFE mask
	and W,W,Y            @ mask off RXFE
	cmp W,#0x0010        @ compare
	beq key1             @ if W == 0x0010 then spin
	ldr W,[X]            @ fetch character
	pushPSP W            @ put it on the stack
	NEXT
	
	defword "BS-IN",5,,BSIN @( n char -- n-1 )
	.int DROP,DUP   @( n n )
	.int ZBRANCH,bsin9
	.int DECR
	.int BS,EMIT,EXIT
bsin9:	.int BELL,EMIT,EXIT

	defword "DEL-IN",6,,DELIN @( n char -- n-1 )
	.int DROP,DUP   @( n n )
	.int ZBRANCH,del9
	.int DECR
	.int BS,EMIT,BL,EMIT,BS,EMIT,EXIT
del9:	.int BELL,EMIT,EXIT

	defword "CR-IN",5,,CRIN @( m addr n char -- m addr m )
	.int DROP         @( m addr n )
	.int SPAN,STORE   @( m addr )
	.int OVER         @( m addr m )
	.int BL,EMIT      @( m addr m )
	.int EXIT
	
	defword "(CHAR)",6,,_CHAR_ @( addr n char -- addr n+1 )
	.int THREEDUP
	.int EMIT
	.int ADD
	.int STOREBYTE
...
DOCONTROL:	@( m addr n char -- m addr x )
	.int DOCOL
	.int DUP,BS,EQU      @( m addr n char f )
	.int ZBRANCH,dochar1
	.int BSIN,EXIT       @( m addr n-1 )
dochar1:	
	.int DUP,cr,EQU 
	.int ZBRANCH,dochar2
	.int CRIN,EXIT       @( m addr m )
dochar2:	
	.int DROP,EXIT
...
DOCHAR:
	.int DOCOL
	.int DUP,LIT,127,EQU  @( m addr n char f )
	.int ZBRANCH,dochar3
	.int DELIN,EXIT
dochar3:	
	.int _CHAR_,EXIT
	...
	defword "EXPECT",6,,EXPECT @( addr n -- ) Get a string from the terminal
	.int DUP,SPAN,STORE
	.int SWAP,ZERO         @( n addr 0 )
expect1:
	.int STWO               @( n addr 0 n )
	.int OVER               @( n addr 0 n 0 )
	.int SUB                @( n addr 0 n-0 )
	.int ZBRANCH,expect9
	.int KEY                @( n addr 0 char )
	.int DUP,BL,LT          @( n addr 0 char f )
	.int ZBRANCH,expect7
	.int DOCONTROL          @( n addr m )
	.int BRANCH,expect1
expect7:
	.int DOCHAR             @( n addr m ) m is n+1
	.int BRANCH,expect1
expect9:
	.int THREEDROP
	.int EXIT

	defword "QUERY",5,,QUERY @( -- )
	.int TIB,LIT,80,EXPECT
	.int SPAN,FETCH
	.int NTIB,STORE
	.int EXIT

	defcode "SKIP",4,,SKIP @( addr len char -- addr1 len1 ) Given the address and length of a string
	popPSP X
	popPSP N
	popPSP W
skip1:	cmp N,#0
...
skip9:	pushPSP N        @ put len onto stack
	pushPSP W        @ put addr onto stack
	NEXT
	
	defcode "SCAN",4,,SCAN @( addr len char -- addr1 len1 ) Given the address and length of a string
	popPSP X
	popPSP N
	popPSP W
scan1:	cmp N,#0
...
scan9:	pushPSP N        @ put len onto stack
	pushPSP W        @ put addr onto stack
	NEXT
	...
	defword "PLACE",5,,PLACE @( from len to -- )
	.int TWODUP              @( from len to len to )
	.int STOREBYTE           @( from len to ) 
	.int INCR                @( from len to+1 )
	.int SWAP                @( from to+1 len )
	.int CMOVE               @( )
	.int EXIT

	defword "PARSE-WORD",10,,PARSEWORD @( char -- addr len )
	.int TOR
	.int TIB,NTIB,FETCH                @( addr n )
	.int SWAP,OVER                     @( n addr n )
	.int TOIN,FETCH                    @( n addr n m )
	.int SLASHSTRING                   @( n addr1 len1 )
	.int RFETCH                        @( n addr1 len1 char )
	.int SKIP                          @( n addr2 len2
	.int OVER                          @( n addr2 len2 addr2 )
	.int SWAP                          @( n addr2 addr2 len2 )
	.int FROMR                         @( n addr2 addr2 len2 char )
	.int SCAN                          @( n addr2 addr3 len3 )
	.int TOR,OVER,SUB,ROT,FROMR
	.int DUP,ZNEQU,ADD,SUB,TOIN,ADDSTORE
	.int EXIT
@	/*
...
@
	...
	defcode ".H16",4,,DOTH16
	popPSP W
	b h16

	defcode ".H32",4,,DOTH32 @ ( x -- )
	popPSP W
	asr X,W,#28
	bl emith4
	asr X,W,#24
...
doth99:	
	...
	.int DEPTH            @ ( x )
	.int SZ               @ ( x addr )
dots1:	.int OVER             @ ( x addr x )
	.int ZBRANCH,dots7    
	.int INCR4,DUP        @ ( x addr+4 addr+4 )
	.int FETCH,DOTH32     @ ( x addr+4 )
	.int BL,EMIT
	.int SWAP,DECR,SWAP   @ ( x-- addr+4 )
	.int BRANCH,dots1
dots7:
	.int DROP,DROP
	.int EXIT
	...
	popPSP W
emit0:	loadREG X,0x4000C000
	strb W,[X]
	NEXT

	defcode "PUTS",4,,PUTS @ ( addr -- )
	loadREG X,0x4000C000
	popPSP W
puts2:	ldrb Y,[W]
	cmp Y,#0
...
@	defcode "BL",2,,BL @ ( -- 0x20 ) put space on stack
@	movw W,#0x20
@	pushPSP W
@	NEXT

	defcode "cr",2,,cr @ ( -- 0x0D ) put 0x0D on stack
	movw W,#0x0D
	pushPSP W
	NEXT

	defcode "bs",2,,bs @ ( -- 0x08 ) put 0x08 on stack
	movw W,#0x08
	pushPSP W
	NEXT

	defcode "CR",2,,CR @ ( -- ) emit CR
	movw W,#0x0D
	loadREG X,0x4000C000
	strb W,[X]
	NEXT

	defcode "NL",2,,NL @ ( -- ) emit NL
	movw W,#0x0A
	loadREG X,0x4000C000
	strb W,[X]
	NEXT
...
*/
	...
	@ my version of BRANCH and 0BRANCH use addresses NOT offsets !!!
...
br0:	ldr FIP,[FIP]
	...
	defcode "0BRANCH",7,,ZBRANCH  @( x -- ) branch if x==0
	popPSP W
	cmp W,#0
	beq br0
	add FIP,FIP,#4
	...
	defcode "TBRANCH",7,,TBRANCH @( x -- ) branch if x!=0
	popPSP W
	cmp W,#0
	bne br0
	add FIP,FIP,#4
...
*/
	...
	@ NOTE: WORD always starts its search at the beginning ot the TIB
	@       NOT ANY LONGER !!! 
	
	defcode "WORD",4,,WORD @ ( addr -- addr n )
	popPSP Z
	DECR Z
word0:	INCR Z
	ldrb X,[Z]     @ fetch char
	cmp X,#0       @ check for null char
	beq word9      @ handle null char
	cmp X,#0x5C    @ check for comment char '\'
	beq word9      @ handle comment char (same as null char
	cmp X,#0x20    @ space
	beq word0      @ spin
	pushPSP Z      @ put addr onto stack
word1:	INCR Z
	ldrb X,[Z]
...
word8:	ldr W,[PSP]
	sub W,Z,W
	pushPSP W      @ put n onto stack
	NEXT
word9:
	pushPSP Z      @ put addr onto stack
	movw W,#0
	pushPSP W      @ put 0 onto stack
	NEXT
...
*/
	...
	defcode "NUMBER",6,,NUMBER  @ ( addr n --  x m  ) m is the number of unconverted characters
	                            @                      if n==0 then we a valid number on the stack
	popPSP N           @ fetch number of characters in string
	popPSP Z           @ fetch address of string
	pushPSP FIP        @ save FIP on FORTH stack so I can use r4 in sub-routine
	bl _NUMBER
	popPSP FIP
	pushPSP Z
	pushPSP N
...
_NUMBER:
	...
	cmp N,#0          @ trying to parse a zero-length string is an error, but will return 0.
	beq num9          @ 

@	loadREG Y,var_BASE @	movl var_BASE,Y	// get BASE (in %dl)
@	ldr Y,[Y]
	movw Y,#10

@	// Check if first character is '-'.
	ldrb r4,[Z]  @ fetch first character into r4 	movb (Z),%bl		// %bl = first character in string
	INCR Z
	pushPSP W	@	// push 0 on stack
	cmp r4,#0x2D	@	cmpb $'-',%bl		// negative number?
	bne num2	@	jnz 2f
@	pop W
@	push X		// push <> 0 on stack, indicating negative
@	dec N
@	jnz 1f
@	pop X		// error: string is only '-'.
@	movl $1,N
@	ret
@
@	// Loop reading digits.
@1:	imull Y,W		// W *= BASE
@	movb (Z),%bl		// %bl = next character in string
@	inc Z
@
...
/*
	...
	We're building up to our prelude on how FORTH code is
	compiled, but first we need yet more infrastructure.

	The FORTH word FIND takes a string (a word as parsed by WORD
	-- see above) and looks it up in the dictionary.  What it
	actually returns is the address of the dictionary header, if
	it finds it, or 0 if it didn't.

	So if DOUBLE is defined in the dictionary, then WORD DOUBLE
	FIND returns the following pointer:
...
*/
	...
	defcode "LFA&N",5,,LFAN @ ( addr -- addr n ) MAKE HEADLESS
	ldr W,[PSP]
	add W,W,#4
	ldrb W,[W]
	movw X,#0x001F
	...
	defcode "S=",2,,STRINGEQU @ ( addr n ADDR -- addr n ADDR f ) compare 2 strings
	mov Z,PSP
	ldr W,[Z]     @ W=ADDR (this address needs to be adjusted)
	add W,W,#5    @ now points to name
	sub Z,Z,#4
	ldr N,[Z]     @ X=n
	sub Z,Z,#4
	ldr Y,[Z]     @ Y=addr
                       @ done with Z at this point
str0:	cmp N,#0
	beq strexit
	ldrb X,[W]    @ fetch char
	ldrb Z,[Y]    @ fetch char
...
strexit:
	...
	defcode "S1",2,,SONE   @ ( v w x y z -- v w x y z y )
	movw X,#4              @         |
	b sx
	defcode "S2",2,,STWO   @ ( v w x y z -- v w x y z x )
	movw X,#8
	b sx
	defcode "S3",2,,STHREE @ ( v w x y z -- v w x y z w )
	movw X,#12
	b sx
	defcode "S4",2,,SFOUR  @ ( v w x y z -- v w x y z v )
	movw X,#16
	b sx
...
sx:	mov W,PSP
	...
	defcode "UPC",3,,UPC @( a -- A ) force char to UPPER case
	popPSP W     @ get char into W
	cmp W,#0x61  @ if ( c < 'a') there is nothing to do
	blo upcdone
	cmp W,#0x7B
	blo upcup
upcdone:	
...
upcup:	sub W,W,#0x20
	...
	defcode "DNC",3,,DNC @( A -- a ) force char to LOWER case
	popPSP W     @ get char into W
	cmp W,#0x41  @ if ( c < 'a') there is nothing to do
	blo dncdone
	cmp W,#0x5B
	blo dncdn
dncdone:	
...
dncdn:	add W,W,#0x20
	...
	defword "FIND",4,,FIND    @ ( addr n -- lfa )
	.int LATEST,FETCH         @ ( addr n ADDR )
find0:
	.int DUP,ZEQU,INVERT
	.int ZBRANCH,find9
	.int LFAN                 @ ( addr n ADDR N )
	.int STWO,EQU             @ ( addr n ADDR f )
	.int INVERT,ZBRANCH,find1
	.int FETCH,BRANCH,find0
find1:	                          @ ( addr n ADDR )
	.int STRINGEQU            @ ( addr n ADDR f )
	.int INVERT,ZBRANCH,find9
	.int FETCH,BRANCH,find0

find9:	                          @ ( addr n ADDR )
	.int SWAP,DROP,SWAP,DROP  @ ( ADDR )
	.int EXIT
@
@
@	+---+---+---+---+---+---+---+
...
@
@
	defcode "COUNT",5,,COUNT @( addr -- addr+1 len
	popPSP W
	ldrb X,[W]
	add W,W,#4
	pushPSP W
...
type9:	NEXT
	...
	defcode "CDUMP",5,,CDUMP @( addr -- addr+16 )
	loadREG Z,0x4000C000
	popPSP W       @ fetch addr into W
	movw N,#16     @ set count
cdump1:	cmp N,#0       @ test for 0
	beq cdump9
	ldrb X,[W]     @ fetch char
	cmp X,#0x20    @ if less that SPACE emit a dot
	blo cdump7
	cmp X,#0x7F    @
	blo cdump5
	b cdump7
...
cdump9:	pushPSP W
	...
	defword "XFIND",5,,XFIND    @ ( addr -- cfa f )
	.int COUNT                @ ( addr+1 n )
	.int LATEST,FETCH         @ ( addr n ADDR )
xfind0:	.int DUP                  @ ( addr n ADDR ADDR )
	.int ZBRANCH,xfind9        @ ( if ADDR is zero we have reached the end of the linked list
	.int LFAN                 @ ( addr n ADDR N )
	.int STWO,EQU             @ ( addr n ADDR f )
	.int INVERT,ZBRANCH,xfind1
	.int FETCH,BRANCH,xfind0
xfind1:	                          @ ( addr n ADDR )
	.int STRINGEQU            @ ( addr n ADDR f )
	.int INVERT,ZBRANCH,xfind9
	.int FETCH,BRANCH,xfind0

xfind9:  @ ( addr n 0 )
	.int SWAP,DROP  @ ( addr 0 )
	.int EXIT
...
*/
	...
	@ BUG: Because I am using null terminated strings I have an "extra" character that I
	@      need to deal with. The name has an additional byte, the null character
	@      Also forgot to count the flags/length byte
	@
	@      This function could be optimized a bit !!!!
	
	defcode ">CFA",4,,TOCFA  @ ( lfa -- cfa )
	popPSP W
	add W,W,#4         @ W points to flags/length field
	ldrb X,[W]         @ X holds flags/length 
	movw Y,#F_LENMASK  @ mask off length
	and X,X,Y
	add W,W,X          @ add length to address
	add W,W,#1         @ BUG: account for the null byte
	add W,W,#1         @ BUG: account flags/length byte
	add W,W,#3
	loadREG X,0xFFFFFFFC @ FC => 1111 1100
	and W,W,X
	pushPSP W
...
*/
	...
	defword ">DFA",4,,TDFA
	.int TOCFA		@ >CFA		(get code field address)
...
*/
	...
	defword "C,",2,,CCOMMA  @ ( x -- ) compile x into dictionary
	.int HERE,FETCH        @ ( x addr )
	.int STORE             @ ( )
	.int HERE,FETCH,INCR   @ ( addr )
	.int HERE,STOREBYTE    @ ( )
	.int EXIT

@	defword ",",1,,COMMA  @ ( x -- ) compile x into dictionary
@	.int HERE,FETCH        @ ( x addr )
@	.int STORE             @ ( )
@	.int HERE,FETCH,INCR4  @ ( addr )
@	.int HERE,STORE        @ ( )
@	.int EXIT
	
	defword "CREATE",6,,CREATE @ ( addr n  -- )
	.int DUP,ZBRANCH,create9 @ if n==0 there is nothing to create
	.int LATEST,FETCH        @ ( a1 n link )
	.int COMMA               @ ( a1 n )          compile link
	.int DUP,CCOMMA          @ ( a1 n )          compile n
	.int OVER,OVER           @ ( a1 n a1 n )
	.int HERE,FETCH,SWAP     @ ( a1 n a1 a2 n )
	.int CMOVE               @ ( a1 n )
	.int HERE,FETCH,ADD      @ ( a1 x )
	.int ALIGN,HERE,STORE    @ ( a1 )
        .int DROP
	.int EXIT
create9:
	.int TWODROP
...
*/
	...
	defcode ",",1,,COMMA  @ ( x -- )
	popPSP X              @ fetch x off the stack
	loadREG W,var_HERE    @ load address of HERE into W
	ldr Y,[W]             @ fetch value of HERE
	str X,[Y]             @ store X into dictionary
	add Y,Y,#4            @ increment
	str Y,[W]
	NEXT
...
*/
	...
	defcode "[",1,F_IMMED,LBRAC @( -- ) set state to ZERO 
	loadREG W,var_STATE
	movw X,#0
	str X,[W]
	NEXT
...
/*
	...
	+-------------------------------------------------------------------------------
	              ...
	              |
	              |
	              this address goes on stack
*/

	defcode "LITSTRING",9,,LITSTRING
	mov W,FIP
	pushPSP W
lits0:	ldrb X,[W]     @ fetch char
	cmp X,#0       @ check for ZERO
	beq lits9
	INCR W
	b lits0
lits9:
	INCR W               @ increment past the null char
	add W,W,#3           @ now align
	loadREG X,0xFFFFFFFC @ FC => 1111 1100
	and W,W,X
	mov FIP,W	
...
*/
	defword "RUN",3,,RUN @( -- )
@	.int EXIT
run6:	.int STATE,FETCH
	.int ZBRANCH,run9
	.int RBRAC
	.int STATE,FETCH,INVERT
	.int ZBRANCH,run7
	.int INTERPRET
run7:
run9:
...
/* END OF jonesforth.S */
	...
	defcode "COFFEE",6,,coffee
	loadREG W,0xC0FFEE
	pushPSP W
	NEXT

@	defword "puts",4,,puts
@	.int DUP,TCFA,DROP,LIT,4,ADD
@	.int INCR
@	.int LIT,0x20002000,STORE
@	.int EXIT

	
	defword "WORDS",5,,WORDS
	.int CR,NL
	.int LIT,last,FETCH,BRANCH,words2
words1:	.int DUP,LIT,5,ADD,PUTS,BL,EMIT,FETCH
words2:	.int DUP,ZEQU,ZBRANCH,words1
	.int DROP
	.int EXIT
...
@	.int PUTS
	...
	defcode "DECIMAL",7,,DECIMAL @ ( -- ) base = 10
	loadREG W,var_BASE
	movw X,#10
	str X,[W]
	NEXT

	defcode "HEX",3,,HEX @ ( -- ) base = 16
	loadREG W,var_BASE
	movw X,#16
	str X,[W]
	NEXT
...
initvars:
	.int DOCOL
	.int ZERO,STATE,STORE                  @ 0 STATE !
	.int DECIMAL                           @ 10 BASE !
	.int SZ,PSPSTORE                       @ S0 PSP!
	.int LIT,last,FETCH,LATEST,STORE       @ last @ LATEST !
	.int LIT,DICTIONARY_SPACE,HERE,STORE   @ HERE = ???
	.int EXIT
...
runforth:
	...
	.int RZ,RSPSTORE	@ R0 RSP!, clear the return stack
rf:	.int DOTOK
	.int ACCEPT            @( addr n ) *** my ACCEPT return only n
	.int DROP              @ I drop n because INTERPRET does not neen it
...
@ ** since ACCEPT never returns zero this line is not needed !!!
	...
	.int EXIT
inter2:	.int TIB
inter1:	.int WORD                        @ ( addr n )
	.int DUP,ZBRANCH,inter10         @ if n == 0 were done !!!
	.int TWODUP,N,STORE,X,STORE      @ save addr n for next parse
	.int FIND                        @ ( lfa )
	.int DUP,ZBRANCH,inter9          @ if lfa==0 word not found, convert to number
	.int TOCFA                       @ ( cfa )
	.int EXECUTE                     @ ( )
	.int X,FETCH,N,FETCH,ADD                     @ ( addr )
	.int BRANCH,inter1               @ spin
inter9:
	.int WHAT
	.int DROP
	.int EXIT
...
@	.int EXIT
	...
	@ BUG: use STOREBYE not STORE !!!
	
	defword ">TIB",4,,TOTIB  @ ( c -- ) place char into TIB
	.int TIB,IN,FETCH,ADD,STOREBYTE
	.int ONE,IN,ADDSTORE
@	.int ZERO,TIB,IN,FETCH,ADD,STOREBYTE    @ place a null character into TIB
	.int EXIT

	
	defcode "EXECUTE",7,,EXECUTE @ ( addr -- )
	popPSP W
	ldr X,[W]
	add X,X,#1
	bx X
	...
	defword "ACCEPT",6,,ACCEPT @ ( -- n ) n is the number of characters in the tib
	.int ZERO,IN,STORE                       @ ALWAYS clear IN
acc0:	.int KEY                                 @ fetch a character
	.int UPC
@	.int DUP,cr,EQU,INVERT,ZBRANCH,acccr     @ ( x
	.int DUP,cr,EQU,TBRANCH,acccr            @ ( x 
@	.int DUP,bs,EQU,INVERT,ZBRANCH,accbs     @ ( x
	.int DUP,bs,EQU,TBRANCH,accbs            @ ( x
	.int DUP,TOTIB                           @ ( x ) place char into TIB
	.int EMIT,BRANCH,acc0                    @ ( -
accbs:	.int DROP,BRANCH,acc0
acccr:
	.int DROP
	.int ZERO,TOTIB   @ place null in TIB  **
	.int IN,FETCH
	.int CR,NL
	.int EXIT
	@ ** by placing a null into TIB, ACCEPT will never return with ZERO !!

	defword "xDUMP",5,,xDUMP  @ ( addr -- addr+16 )
@	.int PAD,DUP,INCR4,SWAP,STORE
	.int FOUR,SWAP          @ ( 4 addr )
	.int DUP,DOTH32
	.int LIT,0x3A,EMIT
	.int BL,EMIT
dump1:	.int DUP,FETCH
@	.int DUP,ITOPAD
	...
	.INT BL,EMIT
	.int INCR4              @ ( 4 addr+4 )
	.int SWAP,DECR,SWAP     @ ( 3 addr )
	.int OVER,ZBRANCH,dump7
	.int BRANCH,dump1
dump7:
	.int LIT,16,SUB
	.int CDUMP
	...
	defword "DIGIT",5,,DIGIT @( x -- c ) convert number to ascii character
	                         @
	                         @ : DIGIT 9 OVER < 7 AND + 48 + ;
	
	.int LIT,9,OVER,LT
	.int LIT,7,AND,ADD,LIT,48,ADD
	.int EXIT

	defword "EXTRACT",7,,EXTRACT @( n base -- n c )
	                             @
...
@	   +-----------------------------------------------------
	...
	defword "<#",2,,LTPOUND @( -- ) initialize HLD
	.int PAD,LIT,32,ADD,HLD,STORE
	.int EXIT

	defword "HOLD",4,,HOLD @( c -- ) put character into buffer
	                       @         the string is built backwards
	                       @ : HOLD HLD @ 1 - DUP HLD STORE C! ;
	
	.int HLD,FETCH,ONE,SUB,DUP,HLD,STORE,STOREBYTE
	.int EXIT

	defword "#",1,,POUND @( u -- u )
	.int BASE,FETCH,EXTRACT,HOLD,EXIT

	defword "#S",2,,POUNDS @( u -- 0 )
pou1:	.int POUND,DUP
	.int TBRANCH,pou1
	.int EXIT
...
sign9:	.int EXIT
	...
	defword "#>",2,,POUNDGT @( x -- addr len
	                        @
	                        @ : #> DROP HLD @ PAD OVER - ;
	.int DROP,HLD,FETCH,PAD,LIT,32,ADD,OVER,SUB,EXIT


	defword "str",3,,STR
	.int DUP,TOR,ABS,LTPOUND,POUNDS,FROMR,SIGN,POUNDGT,EXIT
	
	defword "U.",2,,UDOT     @( u -- )
	                         @
	                         @ : U. <# #S #> TYPE BL EMIT ;
	.int LTPOUND,POUNDS,POUNDGT
	.int TYPE,BL,EMIT
	.int EXIT

	defword ".",1,,DOT     @( n -- addr len )
	                       @
	                       @ : . DUP ABS <# #S  SWAP SIGN #>
	.int DUP,ABS,LTPOUND,POUNDS,SWAP,SIGN,POUNDGT
	.int TYPE,BL,EMIT
	.int EXIT
...
*/
	...
	defword "NUMBER?",7,,NUMBERQ
	.int BASE,FETCH,TOR,ZERO,OVER,COUNT  @( a 0 b n )
	.int OVER,FETCHBYTE,LIT,36,EQU       @( a 0 b n f ) is 1st char $
	.int ZBRANCH,numberq2
	.int HEX,SWAP,ONE,ADD,SWAP,ONE,SUB   @( a 0 b' n' )
numberq2:
	.int OVER,FETCHBYTE,LIT,45,EQU,TOR   @( a 0 b n )
	.int SWAP,RFETCH,SUB,SWAP,RFETCH,ADD @( a 0 b' n' )
	.int QDUP
	.int ZBRANCH,numberq4
	.int
	
numberq4:	


	defcode "R@--",3,,RFETCHDECR @( x -- ) R( x-1 )
	popRSP X
	pushPSP X
	DECR X
	pushRSP X
	...
	defword "DUMP",4,,DUMP @( addr lines -- )
	.int TOR           @( addr )
dump2:	.int RFETCHDECR    @( addr x )
	.int ZBRANCH,dump9
	.int xDUMP,CR,NL
	.int BRANCH,dump2
dump9:	.int FROMR,TWODROP,EXIT
...
@	.int LIT,0X41,LIT,0X4000C000,STOREBYTE
	.int LIT,last,FETCH,LFAN
	
	.int LIT,0x20002000        @ ( addr )
	.int FOUR,OVER             @ ( addr 4 addr )
	.INT STOREBYTE             @ ( addr
	.int INCR,FOUR,OVER
	.INT STOREBYTE             @ ( addr
	.int INCR,FOUR,OVER
	...
	.INT STOREBYTE             @ ( addr
	.int ONE,TWO,THREE
	.int ROT,ROT,ROT,NROT,NROT,NROT
	.int OVER,SWAP,ADD,ADD,INCR,INCR,INCR
	.int DUP
	.int DROP,DROP,DROP
@	.int coffee
@	.int WORDS
YYY:	.int DOTOK
	...
	.int DROP     @ I drop n because INTERPRET does not neen it 
@ **	.int DUP,ZBRANCH,zzz
	.int INTERPRET,BRANCH,YYY
	.int WORD
@	.int 2DUP,ADD,X,STORE
	.int FIND,NOOP
	.int TOCFA,EXECUTE
	.int BRANCH,YYY
XXX:	.int KEY,EMIT,BRANCH,XXX
	.int EXIT
zzz:	.int DROP,DROP,BRANCH,YYY
...
digit09: @( char -- n f )
@	defcode "xDIGIT",6,,xDIGIT @( char base -- n f )
	.int digit09+4
	popPSP X        @ char into X
	sub X,X,#0x30   @ sub ascii 0 from X if less than 0 char is NOT a digit
	bmi digitfail   @ MI = negative
	cmp X,#10
	blo digit09yes
digitfail:
	movw W,#0
...
digitAF: @( char -- n f )
	.int digitAF+4
	popPSP X        @ char into X
	sub X,X,#0x41   @ sub ascii A from X if less than 0 char is NOT a hex digit
	bmi digitfail   @ MI = negative
	cmp X,#5
	blo digitAFyes
	b digitfail
digitAFyes:
	...
	@ this was tricky BUT I think I have it !!!
	@
	defcode "DIGIT?",6,,DIGITQ @( char base -- n f )
digq:	popPSP W           @ get base into W
	popPSP X           @ get char into X
	sub X,X,#48        @ convert char to number  ascii 48 => 0
	                   @                         ascii 49 => 1
	                   @                         ascii 50 => 2
	                   @                           ...
	                   @                         ascii 57 => 9
	cmp X,#10          @ if ( X < 9 ) then we have our number
	BLT have_digit
...
have_digit:
	pushPSP X
check_base:	
dig8:	cmp X,W      @ if ( X < base ) we have a good number, otherwise put F on stack
	blo digT
digF:	movw W,#0
	pushPSP W
	NEXT
...
dig10:	pushPSP X
	...
	defword "TEST1",5,,TEST1
	.int LIT,0x23,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x30,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x36,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x39,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x3A,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x41,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x44,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x46,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x47,LIT,10,DIGITQ,DOTH32,DOTH32,CR,NL
	.int EXIT

	defword "TEST2",5,,TEST2
	.int LIT,0x23,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x30,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x36,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x39,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x3A,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x41,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x44,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x46,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int LIT,0x47,LIT,16,DIGITQ,DOTH32,DOTH32,CR,NL
	.int EXIT
